...
class OUTput:
    ...
    def output_text(self, output=False):
        ...
        for list_data_vertical in self.list_all_data_vertical:
            for data_vertical in list_data_vertical:
                if len(data_vertical) == 0:
                    ...
                    if output == "terminal":
                        print(data_vertical)
                        data_vert.append(data_vertical)
                    if output == "csv":
            ...
            for data_gorizontal in list_data_gorizontal:
                if len(data_gorizontal) == 0:
                    ...
                    if output == "terminal":
                        ...
                        data_gor.append(data_gorizontal)
                    if output == "csv":
        ...
        if output == "csv":
            ...
            # print(text_recognition_gorizontal, "".join(new_list_text_recognition_gorizontal))
    ...
    def write_csv(self, abs_path_csv, base_64, text_recognition_vertical, text_recognition_gorizontal):
        for face_descriptor in self.list_face_descriptor:
            ...
            dfdata = {
                    ...
                    "other_data": text_recognition_gorizontal
            }
        #     data = {
        ...
        if abs_path_csv.split('/')[-1] in os.listdir():
            df = pd.read_csv(abs_path_csv.split('/')[-1], index_col = 0)
            df.loc[len(df.index)] = dfdata.values()
        else:
            df = pd.DataFrame({i: [dfdata[i]] for i in dfdata})
        df.to_csv(abs_path_csv.split('/')[-1])
    ...
    def main(self):
        if self.output == "terminal":
            ...
            (data_vertical, data_gorizontal) = self.output_text(output="terminal")
            self.del_folder(output="terminal")
            return data_vertical, data_gorizontal
